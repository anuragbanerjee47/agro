...
def run_simulator():
    ...
    while True:
        ...
        try:
            res = requests.post(f"{SERVER_URL}/api/telemetry", json=payload, timeout=5)
            print(f"[Telemetry] Status: {res.status_code} | NPK: {payload['soil_npk']} | pH: {payload['ph']} | Moisture: {payload['moisture']}%")
        ...
        try:
            ...
            upload_res = requests.post(f"{SERVER_URL}/api/upload", data=data, files=files, timeout=5)
